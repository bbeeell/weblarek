...
Конструктор:  
`constructor(events: IEvents)` - принимает объект брокера событий для уведомления системы об обновлениях в корзине.

Поля класса:  
`protected storedItems: IProduct[]` - массив всех товаров, выбранных покупателем для покупки, которые находятся в корзине.
`protected eventBus: IEvents` - объект для отправки событий.

Методы:  
`retrieveAllItems(): IProduct[]` - возвращает массив всех товаров, которые находятся в корзине.
`pushItem(newItem: IProduct): void` - добавляет товар в массив корзины. Выполняет проверку на наличие дубликатов (товар не будет добавлен повторно). Генерирует событие `cart:item-added`.
`discardItem(targetId: string): void` - удаляет товар с указанным id из массива корзины с помощью фильтрации. Генерирует событие `cart:item-removed`.
`resetCart(): void` - очищает корзину, устанавливая пустой массив. Генерирует событие `cart:cleared`.
`evaluateTotalPrice(): number` - вычисляет и возвращает общую стоимость всех товаров в корзине путем суммирования их цен. Возвращает 0 для пустой корзины (или для корзины с бесценными товарами).
`countItems(): number` - возвращает количество товаров в корзине через свойство length массива.
`verifyItemExistence(itemId: string): boolean` - проверяет наличие товара с указанным id в корзине, возвращает true если товар найден, и false - если нет.
...
Конструктор:  
`constructor()` - инициализирует все поля данных покупателя с пустыми значениями.

Поля класса:  
`paymentType: string` - выбранный способ оплаты.
`deliveryAddress: string` - адрес доставки.
`contactEmail: string` - электронная почта покупателя.
`contactPhone: string` - телефонный номер покупателя.

Методы:  
`resetProfile(): void` - очищает все данные покупателя, сбрасывая все поля к пустым значениям.
`validateFields(): Record<string, string>` - проверяет валидность всех полей покупателя. Для каждого поля возвращает пустую строку если поле валидно, или текст ошибки если поле невалидно. Поле считается валидным если оно не пустое после удаления пробелов в начале и конце.

### Слой коммуникации

#### Класс ServerApi
Является представителем коммуникационного слоя — отвечает за получение данных с сервера и отправку данных на сервер. Использует композицию для выполнения запросов через переданный экземпляр класса Api.

Конструктор:  
`constructor(apiService: IApi)` - принимает объект, соответствующий интерфейсу IApi.

Поля класса:  
`protected requester: IApi` - объект, предоставляющий методы для выполнения запросов на сервер.

Методы:  
`requestProductCatalog(): Promise<IProductList>` - выполняет get запрос на эндпоинт /product/ и возвращает промис с объектом, содержащим массив товаров.
`submitOrder(payload: IOrderData): Promise<IOrderResponse>` - выполняет post запрос на эндпоинт /order/, передает данные заказа и возвращает промис с результатом оформления заказа.
...
Описание: сигнализируют об изменении состава корзины (добавление, удаление или полная очистка).
...
В данном приложении Презентер реализован непосредственно в основном скрипте приложения main.ts без выделения в отдельный класс. Такой подход позволяет гибко управлять логикой в единой точке входа, связывая модели и представления через события, и делает код более простым для понимания в рамках учебного проекта.
